...
int main(int argc, char* argv[])
{
	...
	int nDetections, count1 = 0, count2 = 0;
...
//#endif
#ifdef car_up 
				if ((t.od->classes[i] == 8 || t.od->classes[i] == 6 || t.od->classes[i] == 3) && (t.od->boxes[i].y <= 160) && (t.od->boxes[i].y >= 100) && (t.od->boxes[i].x <= 300) ) {
...
#endif
#ifdef car_down
				if ((t.od->classes[i] == 8 || t.od->classes[i] == 6 || t.od->classes[i] == 3) && (t.od->boxes[i].y >= 185) && ((t.od->boxes[i].x <= 450))) {
					cv::rectangle(image, t.od->boxes[i], cv::Scalar(255, 255, 0), 2);
					count2++;
					std::cout
						<< "DOWN: " << count2 << "\t"
						<< std::endl;
